...
class DynHouseholdLaborModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        # children
        par.n_child = 1
        par.child_grid = np.arange(0, par.n_child)
        par.p_child = 0.1
    ...
    def allocate(self):
        ...
        # d. solution arrays
        shape = (par.T,par.n_child, par.Nk,par.Nk)
                ...
                for i_k1,capital1 in enumerate(par.k_grid):
                    for i_k2,capital2 in enumerate(par.k_grid):
                        ...
                        if t==(par.T-1): # last period
                            obj = lambda x: -self.util(i_n,x[0],x[1],capital1,capital2)
                        else:
                            obj = lambda x: self.value_of_choice(i_n,x[0],x[1],capital1,capital2,sol.V[t+1])
    ...
    def value_of_choice(self,child,hours1,hours2,capital1,capital2,V_next):

        # a. unpack
        par = self.par

        # b. current utility
        util = self.util(child,hours1,hours2,capital1,capital2)
        ...
        EV_next = 0
        p_birth = [(1-par.p_child), par.p_child]
        for birth in [0,1]:
            if child == par.n_child -1:
                birth = 0

            kids_next = child + birth
            EV_next += p_birth[birth]*interp_2d(par.k_grid,par.k_grid,V_next[kids_next],k1_next,k2_next)
    ...
    def util(self,child,hours1,hours2,capital1,capital2):
        par = self.par
